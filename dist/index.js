...
const btnSubmitTarjeta = document.querySelector("#submitTarjeta");
const btnSubmitMetalico = document.querySelector("#submitMetalico");
// Botones para cerrar los modal
const btnCerrarModalTarjeta = document.querySelector("#btnCerrarModalTarjeta");
const btnCerrarModalMetalico = document.querySelector("#btnCerrarModalMetalico");
// Modals (tarjeta y metalico)
const modalTarjeta = document.querySelector("#modalTarjeta");
const modalMetalico = document.querySelector("#modalMetalico");
// Clicks 
 ...
 */
function validarInputs(metodoPago) {
    console.log(metodoPago);
    if (metodoPago == "tarjeta") {
        var amount = document.getElementById("amountTarjeta");
        var currency = document.getElementById("currencyTarjeta");
        var cardNum = document.getElementById("cardNumTarjeta");
        console.log(currency.value);
        if (amount.value == "" || currency.value == "" || cardNum.value == "") {
            console.log("Faltan campos por informar");
            spanErrorTarjeta === null || spanErrorTarjeta === void 0 ? void 0 : spanErrorTarjeta.classList.remove('hidden');
        }
        else {
            if (validarTarjetaCredito(cardNum.value)) {
                ...
                console.log(res);
                spanErrorTarjeta === null || spanErrorTarjeta === void 0 ? void 0 : spanErrorTarjeta.classList.add('hidden');
        ...
        var amount = document.getElementById("amountMetalico");
        var currency = document.getElementById("currencyMetalico");
        var coin1 = document.getElementById("coin1");
        if (isNaN(parseInt(coin1.value)))
            coin1.value += 0;
        var coin2 = document.getElementById("coin2");
        if (isNaN(parseInt(coin2.value)))
            coin2.value += 0;
        var coin5 = document.getElementById("coin5");
        if (isNaN(parseInt(coin5.value)))
            coin5.value += 0;
        var coin10 = document.getElementById("coin10");
        if (isNaN(parseInt(coin10.value)))
            coin10.value += 0;
        var coin20 = document.getElementById("coin20");
        if (isNaN(parseInt(coin20.value)))
            coin20.value += 0;
        var coin50 = document.getElementById("coin50");
        if (isNaN(parseInt(coin50.value)))
            coin50.value += 0;
        var coin100 = document.getElementById("coin100");
        if (isNaN(parseInt(coin100.value)))
            coin100.value += 0;
        var coin500 = document.getElementById("coin500");
        if (isNaN(parseInt(coin500.value)))
            coin500.value += 0;
        let total = parseInt(coin1.value) * 1 + parseInt(coin2.value) * 2 + parseInt(coin5.value) * 5 + parseInt(coin10.value) * 10
            + parseInt(coin20.value) * 20 + parseInt(coin50.value) * 50 + parseInt(coin100.value) * 100 + parseInt(coin500.value) * 500;
        let cambio = total - parseInt(amount.value);
        ...
        if (amount.value == "" || currency.value == "") {
            ...
            spanErrorMetalico === null || spanErrorMetalico === void 0 ? void 0 : spanErrorMetalico.classList.add('hidden');
